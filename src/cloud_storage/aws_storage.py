...
class SimpleStorageService:
    ...
    # def upload_file(self, from_filename: str, to_filename: str, bucket_name: str, remove: bool = True):
        ...
        try:
            ...
            Filename=from_filename,  # local file path
            Bucket=bucket_name,      # S3 bucket name
            Key=to_filename          # file name in S3
            )
            ...
            #     Filename=from_filename,    # local file path
            #     Bucket=bucket_name,        # S3 bucket name
            #     Key=to_filename            # S3 file name
            # )

            # self.s3.upload_file(
            #     from_filename=self.model_file_path,  # local file path
            #     to_filename="model.pkl",             # file name in S3
            #     bucket_name="my-model-mlopsproj1"   # bucket name only
            # )
            # logging.info(f"Uploaded {from_filename} to {to_filename} in {bucket_name}")

            # Delete the local file if remove sis True
            # if remove:
            #     os.remove(from_filename)
            #     logging.info(f"Removed local file {from_filename} after upload")
            # logging.info("Exited the upload_file method of SimpleStorageService class")
    ...
    def upload_file(self, from_filename: str, to_filename: str, bucket_name: str):
        ...
        logging.info("Entered the upload_file method of SimpleStorageService class")
        try:
            logging.info(f"Uploading {from_filename} to {bucket_name}/{to_filename}")

            # ✅ Correct boto3 usage
            self.s3_client.upload_file(
                Filename=from_filename,  # Local file
                Bucket=bucket_name,      # Correct bucket name
                Key=to_filename          # File name in S3
            )

            logging.info(f"Successfully uploaded {from_filename} to s3://{bucket_name}/{to_filename}")
